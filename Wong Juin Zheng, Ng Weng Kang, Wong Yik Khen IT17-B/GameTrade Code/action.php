...
if(isset($_POST["getProduct"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			$pro_bprice = $row['product_bprice'];
			$pro_image = $row['product_image'];
			$pro_stype = $row['product_stype'];
			$pro_quantity = $row['product_quantity'];
			echo "
				<div class='col-md-4' >
				
							<a href='product_details.php?prodid=".$pro_id."'><div class='panel panel-info'>
								<div class='panel-heading panel-info'>$pro_title</br></div>
								<div class='panel-body'>
									<img src='product_images/$pro_image' style='width:150px; height:200px;'/>
								</div>
							</a>	
								<div class='panel-heading'>
								<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger btn-xs'>AddToCart</button>
									$$pro_price.00</br>
									<s style='color:grey;font-size:12px;'>$$pro_bprice.00</s>
								</div>
							</div>
						</div>	
			";
...
//get cart item from database in dropdown menu
				...
				if (!isset($_SESSION["uid"])) {
					...
					echo '
						...
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							...
							<input type="hidden" name="business" value="eoss01-facilitator@outlook.com">
							...
							echo   
								'<input type="hidden" name="return" value="http://172.16.0.12/D170325B/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://172.16.0.12/D170325B/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://172.16.0.12/D170325B/cancel.php"/>
...
if (isset($_POST["updateCartItem"])) {
	...
	$sql2 = "SELECT product_quantity FROM products WHERE product_id = '$update_id'";
...
if(!empty($_POST['rating']) && !empty($_POST['itemId'])){
	...
	$insertRating = "INSERT INTO item_rating (itemId, userId, username, ratingNumber, title, comments, created, modified) VALUES ('".$itemId."', '".$userID."', '".$username."', '".$_POST['rating']."', '".$_POST['title']."', '".$_POST["comment"]."', '".date("Y-m-d H:i:s")."', '".date("Y-m-d H:i:s")."')";
